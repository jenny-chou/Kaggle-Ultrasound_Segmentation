...
def image_plus_mask(img, mask, pred):
    ...
    img_color[pred_edges, 0] = 0  
    img_color[pred_edges, 1] = 0
    img_color[pred_edges, 2] = 255 # chan 2 = blue
    
    return img_color


def to_pred_path(f_image):
    # Convert an image file path into a corresponding mask file path 
    dirname, basename = os.path.split(f_image)
    maskname = basename.replace(".tif", "_pred.png")
...
def add_masks(images):
    ...
    for f_image in images:
        ...
        pred = cv2.imread(to_pred_path(f_image))
        images_plus_masks[f_image] = image_plus_mask(img, mask, pred)
...
def get_patient_images(patient):
    ...
    f_ultrasounds = [f for f in glob.glob(f_path) if 'mask' not in f and 'pred' not in f]
...
def write_patient_video(patient):
    ...
    write_gif(f_seq, images_masks, 'patient-%02i_30.gif' % patient)


def plot(patients):
    # Animations for patients    
    for patient in patients:
        write_patient_video(patient=patient)
